...
function Fixture(){const [screen,setScreen]=useState('tickets'),[view,setView]=useState('all'),[board,setBoard]=useState('all'),[company,setCompany]=useState('all'),[totals,setTotals]=useState(summary),[revision,setRevision]=useState(0);return <><aside style={{padding:16,background:'#ececec',fontSize:14}}>Isolated UI fixture — no live records or email. <button onClick={()=>setScreen('tickets')}>Ticket fixture</button> · <button onClick={()=>{history.replaceState({},'',location.pathname+'?org=demo&ticket=1002');setRevision(r=>r+1)}}>Quick-open ticket 1002</button> · <button onClick={()=>setScreen('portal')}>Portal fixture</button> · <button onClick={()=>setScreen('remote')}>Loading fixture</button></aside>{screen==='tickets'?<main style={{padding:24}}><TicketWorkspace org="demo" workspace={workspace} user={user} view={view} setView={setView} board={board} setBoard={setBoard} company={company} setCompany={setCompany} revision={revision} summary={totals} setSummary={setTotals} openDirectory={()=>{}}/></main>:screen==='portal'?<Portal/>:<RemoteProbe/>}</>}

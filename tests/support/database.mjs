...
export class D1 {
 ...
 async batch(statements){this.sql.exec('BEGIN');try{const result=[];for(const s of statements)result.push(await (/^\s*(SELECT|PRAGMA)\b|\bRETURNING\b/i.test(s._sql)?s.all():s.run()));this.sql.exec('COMMIT');return result}catch(e){this.sql.exec('ROLLBACK');throw e}}

...
After those tests and owner policy approval, configure matching LIVE keys/prices/webhook, set QUEVIAN_LEGAL_APPROVED=true and QUEVIAN_BILLING_ENABLED=true, deploy, and perform an owner-authorized live purchase/cancellation check. The owner requested paid-only access on 2026-09-22. Ticketing reads and writes now require an active subscription with a provider subscription ID, an unexpired billing period, and enough purchased seats. Trial, incomplete, unpaid, paused, canceled, and expired subscriptions do not grant access. QUEVIAN_BILLING_REQUIRED is retained as true for configuration clarity; false or missing no longer bypasses enforcement. No charges or account-verification actions were performed by this launch pass.

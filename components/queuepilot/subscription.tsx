...
export function SubscriptionSettings({org,owner}:{org:string;owner:boolean}){
 ...
 <form className="qp-form" onSubmit={e=>{e.preventDefault();act(currentSubscription?'seats':'checkout',currentSubscription?{seats:quantity,confirm:true}:{seats:quantity,interval,acceptedTerms:true})}}>
 ...
 <label className="checkbox-label"><Checkbox checked={consent} onCheckedChange={v=>setConsent(v===true)}/><span>{currentSubscription?'I authorize this seat change and any resulting prorated charge.':<>I agree to the <a href="/terms" target="_blank" rel="noreferrer">terms</a> and recurring billing shown at checkout.</>}</span></label>

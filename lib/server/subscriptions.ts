...
// Only canonical subscription state written by the verified Stripe flow grants access.
export function hasPaidAccess(sub:Pick<Subscription,'subscription_id'|'status'|'period_end'>|null){return !!sub?.subscription_id&&sub.status==='active'&&Number(sub.period_end)>Date.now()/1000}
export async function requirePaidWorkspace(s:Service,org:string){const row=await s.one<Subscription>('SELECT * FROM workspace_subscriptions WHERE org_id=?',org);if(!hasPaidAccess(row))throw new HttpError(402,'An active paid subscription is required to access this workspace. The workspace owner can subscribe or update payment details.');if(await seatUsage(s,org)>(row?.seats??0))throw new HttpError(402,'Your team exceeds the purchased seats. The workspace owner must add seats before continuing.');}

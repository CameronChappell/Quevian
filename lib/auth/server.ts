...
return null;}
export function safeNext(value:unknown,fallback='/app'){if(typeof value!=='string')return fallback;try{const u=new URL(value,'https://quevian.invalid');if(u.origin!=='https://quevian.invalid'||!(['/app','/portal'].includes(u.pathname)||/^\/invite\/[a-zA-Z0-9-]{1,150}$/.test(u.pathname)))return fallback;return u.pathname+u.search}catch{return fallback}}
export async function requireAccount(next:string){const user=await accountIdentity();if(!user)redirect('/login?next='+encodeURIComponent(safeNext(next)));if(!env.DB)throw new HttpError(503,'Shared storage is unavailable.');if(!await hasAcceptedTerms(env.DB,user.id))redirect('/review-terms?next='+encodeURIComponent(safeNext(next)));return user}
